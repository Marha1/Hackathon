...
public class AdsService : IAdsService
{
    ...
    {
        var ads = await _adsRepository.GetAll();
        return ads.Where(ad => ad.Text == text).Select(ad => _mapper.Map<AdsGetByTextResponce>(ad));
    }
    ...
    public async Task<IEnumerable<AdsFiltrationResponce>> Filtration(bool ascending)
    {
        ...
        return ads == null
            ? null
            : (ascending ? ads.OrderBy(n => n.Rating) : ads.OrderByDescending(n => n.Rating))
            .Select(ad => _mapper.Map<AdsFiltrationResponce>(ad));
    }
}